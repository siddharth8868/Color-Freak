...
public class Survive extends Activity implements OnClickListener, OnCompletionListener {
    ...
    String play;
    Intent im;
    int qs,as,correct=0,wrong=0,qno=0,colno,i,lenght=7,time,got=0,score=0,ttime=0;
    MediaPlayer mp;
    CountDownTimer counttime,ct1,ct2;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        correct=i.getIntExtra("cc",0);
        wrong=i.getIntExtra("w",0);
        got=i.getIntExtra("g",10000);
        qno=i.getIntExtra("q",0);
	...
	private void cal(int ttp) {
		// TODO Auto-generated method stub
		got=0;
		
counttime=new CountDownTimer(ttp,1000) {
			...
			public void onFinish() {
				...
				im=new Intent(getApplicationContext(),Result.class);
			if(got==0)
			{
				Toast.makeText(getApplicationContext(),"inside got",Toast.LENGTH_LONG).show();

				qno--;
				im.putExtra("q",qno);
				im.putExtra("c",correct);
				im.putExtra("w",wrong);
				im.putExtra("time",time/1000);
				im.putExtra("play", play);
				score=score+(correct*10)-(wrong*5)-(time/1000);
				
				im.putExtra("score",score);
			...
			{
				Toast.makeText(getApplicationContext(),"extra time="+got,Toast.LENGTH_LONG).show();

				im=new Intent(getApplicationContext(),Survive.class);
				
				im.putExtra("q",qno);
		        im.putExtra("time",got);
		        im.putExtra("play",play);
		        im.putExtra("cc",correct);
		        im.putExtra("w",wrong);
		        im.putExtra("g",got);
		        score=score+(correct*10)-(wrong*5)-(time/1000);
		        im.putExtra("score",score);
		        im.putExtra("ttime",ttime+time);
		        startActivity(im);
		        finish();
